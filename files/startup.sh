...
if [[ -n "${DATANUCLEUS_CONNECTION_POOLING_TYPE}" ]]; then
    ...
    if [[ "${DATANUCLEUS_CONNECTION_POOLING_TYPE,,}" == 'bonecp' ]]; then
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MAX_POOLSIZE}" ]] && update_property.py datanucleus.connectionPool.maxPoolSize "${DATANUCLEUS_CONNECTION_POOL_MAX_POOLSIZE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MIN_POOLSIZE}" ]] && update_property.py datanucleus.connectionPool.minPoolSize "${DATANUCLEUS_CONNECTION_POOL_MIN_POOLSIZE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MAX_IDLE}" ]] && update_property.py datanucleus.connectionPool.maxIdle "${DATANUCLEUS_CONNECTION_POOL_MAX_IDLE}" /etc/hive/conf/hive-site.xml
    fi

    if [[ "${DATANUCLEUS_CONNECTION_POOLING_TYPE,,}" == 'dbcp' || "${DATANUCLEUS_CONNECTION_POOLING_TYPE,,}" == 'dbcp2' ]]; then
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MAX_IDLE}" ]] && update_property.py datanucleus.connectionPool.maxIdle "${DATANUCLEUS_CONNECTION_POOL_MAX_IDLE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MIN_IDLE}" ]] && update_property.py datanucleus.connectionPool.minIdle "${DATANUCLEUS_CONNECTION_POOL_MIN_IDLE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MIN_ACTIVE}" ]] && update_property.py datanucleus.connectionPool.maxActive "${DATANUCLEUS_CONNECTION_POOL_MIN_ACTIVE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MAX_WAIT}" ]] && update_property.py datanucleus.connectionPool.maxWait "${DATANUCLEUS_CONNECTION_POOL_MAX_WAIT}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_VALIDATION_TIMEOUT}" ]] && update_property.py datanucleus.connectionPool.validationTimeout "${DATANUCLEUS_CONNECTION_POOL_VALIDATION_TIMEOUT}" /etc/hive/conf/hive-site.xml
    fi

    if [[ "${DATANUCLEUS_CONNECTION_POOLING_TYPE,,}" == 'c3p0' ]]; then
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MAX_POOLSIZE}" ]] && update_property.py datanucleus.connectionPool.maxPoolSize "${DATANUCLEUS_CONNECTION_POOL_MAX_POOLSIZE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MIN_POOLSIZE}" ]] && update_property.py datanucleus.connectionPool.minPoolSize "${DATANUCLEUS_CONNECTION_POOL_MIN_POOLSIZE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_INITIAL_POOLSIZE}" ]] && update_property.py datanucleus.connectionPool.initialPoolSize "${DATANUCLEUS_CONNECTION_POOL_INITIAL_POOLSIZE}" /etc/hive/conf/hive-site.xml
    fi

    if [[ "${DATANUCLEUS_CONNECTION_POOLING_TYPE,,}" == 'hikaricp' ]]; then
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MAX_POOLSIZE}" ]] && update_property.py datanucleus.connectionPool.maxPoolSize "${DATANUCLEUS_CONNECTION_POOL_MAX_POOLSIZE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_MIN_IDLE}" ]] && update_property.py datanucleus.connectionPool.minIdle "${DATANUCLEUS_CONNECTION_POOL_MIN_IDLE}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_LEAK_DETECTION_THRESHOLD}" ]] && update_property.py datanucleus.connectionPool.leakThreshold "${DATANUCLEUS_CONNECTION_POOL_LEAK_DETECTION_THRESHOLD}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_LEAK_MAX_LIFETIME}" ]] && update_property.py datanucleus.connectionPool.maxLifetime "${DATANUCLEUS_CONNECTION_POOL_LEAK_MAX_LIFETIME}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_AUTO_COMMIT}" ]] && update_property.py datanucleus.connectionPool.autoCommit "${DATANUCLEUS_CONNECTION_POOL_AUTO_COMMIT}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_IDLE_TIMEOUT}" ]] && update_property.py datanucleus.connectionPool.idleTimeout "${DATANUCLEUS_CONNECTION_POOL_IDLE_TIMEOUT}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_CONNECTION_WAIT_TIMEOUT}" ]] && update_property.py datanucleus.connectionPool.connectionWaitTimeout "${DATANUCLEUS_CONNECTION_POOL_CONNECTION_WAIT_TIMEOUT}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_VALIDATION_TIMEOUT}" ]] && update_property.py datanucleus.connectionPool.validationTimeout "${DATANUCLEUS_CONNECTION_POOL_VALIDATION_TIMEOUT}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_READ_ONLY}" ]] && update_property.py datanucleus.connectionPool.readOnly "${DATANUCLEUS_CONNECTION_POOL_READ_ONLY}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_NAME}" ]] && update_property.py datanucleus.connectionPool.name "${DATANUCLEUS_CONNECTION_POOL_NAME}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_CATALOG}" ]] && update_property.py datanucleus.connectionPool.catalog "${DATANUCLEUS_CONNECTION_POOL_CATALOG}" /etc/hive/conf/hive-site.xml
       [[ -n "${DATANUCLEUS_CONNECTION_POOL_REGISTER_MBEANS}" ]] && update_property.py datanucleus.connectionPool.registerMbeans "${DATANUCLEUS_CONNECTION_POOL_REGISTER_MBEANS}" /etc/hive/conf/hive-site.xml
    fi
fi
    ...
    if [[ -n "$HIVE_DB_NAMES" ]]; then
        ...
        if [[ -n "$AWS_ACCOUNT" ]]; then
          ...
          do
              ...
              if [[ -z "$DISABLE_GLUE_DB_INIT" && -n "$ENABLE_GLUESYNC" ]]; then
...
[[ -n "$HIVE_DB_WHITELIST" ]] && export METASTORE_PRELISTENERS="${METASTORE_PRELISTENERS},com.expediagroup.apiary.extensions.readonlyauth.listener.ApiaryReadOnlyAuthPreEventListener"

[[ -n "$HIVE_METASTORE_LOG_LEVEL" ]] && HIVE_METASTORE_LOG_LEVEL="INFO"
sed "s/HIVE_METASTORE_LOG_LEVEL/$HIVE_METASTORE_LOG_LEVEL/" -i /etc/hive/conf/hive-log4j2.properties

[[ -n "$SNS_ARN" ]] && export METASTORE_LISTENERS="${METASTORE_LISTENERS},com.expediagroup.apiary.extensions.events.metastore.listener.ApiarySnsListener"
[[ -n "$KAFKA_BOOTSTRAP_SERVERS" ]] && export METASTORE_LISTENERS="${METASTORE_LISTENERS},com.expediagroup.apiary.extensions.events.metastore.kafka.listener.KafkaMetaStoreEventListener"
[[ -n "$ENABLE_GLUESYNC" ]] && export METASTORE_LISTENERS="${METASTORE_LISTENERS},com.expediagroup.apiary.extensions.gluesync.listener.ApiaryGlueSync"
...
[[ -n "$ENABLE_GLUESYNC" ]] && export METASTORE_PRELISTENERS="${METASTORE_PRELISTENERS},com.expediagroup.apiary.extensions.gluesync.listener.ApiaryGluePreEventListener"
...
[[ -n "$SNS_ARN" ]] && export AUX_CLASSPATH="$AUX_CLASSPATH:/usr/lib/apiary/apiary-metastore-listener-${APIARY_EXTENSIONS_VERSION}-all.jar"
[[ -n "$KAFKA_BOOTSTRAP_SERVERS" ]] && export AUX_CLASSPATH="$AUX_CLASSPATH:/usr/lib/apiary/kafka-metastore-listener-${APIARY_EXTENSIONS_VERSION}-all.jar:/usr/lib/apiary/kafka-clients-${KAFKA_VERSION}.jar"
...
if [ -n "${ENABLE_HIVE_LOCK_HOUSE_KEEPER}" ]; then
